 ...
 * contained slideshow: it loops through `galleryItems`, responds to arrow
 * keys and swipe, and keeps its own index.
 */
...
function GalleryMedia({
  ...
  return (
    <div
      aria-hidden="true"
      className={`absolute inset-0 overflow-hidden ${placeholderToneClasses[item.tone]}`}
    >
      ...
      <div className="absolute inset-x-5 bottom-5 flex items-end justify-between gap-3 font-mono text-[9px] uppercase tracking-[0.19em] text-[#F5F1E8]/70">
        ...
        <span className="text-right text-[#D66A2B]">
...
export default function GalleryPage() {
  ...
  // No filtering anymore — the slideshow always cycles through everything.
  ...
  const openImage = useCallback(
    (id: string, currentTarget: HTMLElement) => {
  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-[#070707] text-[#F5F1E8] selection:bg-[#C65A24] selection:text-white">
      <style jsx global>{`
        ...
        html.gallery-motion-ready
          [data-gallery-reveal].gallery-revealed {
          opacity: 1;
      ...
      <main id="main-content">
        <section className="relative scroll-mt-28 px-5 pb-16 pt-32 sm:px-8 sm:pt-36 lg:px-12 lg:pb-24 lg:pt-40">
          <div className="pointer-events-none absolute left-[-12rem] top-20 -z-10 h-[34rem] w-[34rem] rounded-full bg-[#C65A24]/10 blur-[120px]" />

          <div className="mx-auto max-w-[1600px]">
            <div
              className="grid gap-7 border-b border-white/10 pb-8 md:grid-cols-12 md:items-end"
              data-gallery-reveal
            >
              <div className="md:col-span-5">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#D66A2B]">
                  Karachi Flames / Gallery
                </p>

                <h1 className="mt-4 max-w-[10ch] font-serif text-[clamp(2.8rem,6.3vw,6.8rem)] leading-[0.82] tracking-[-0.07em] text-[#F5F1E8]">
                  LOOK{" "}
                  <span className="italic text-[#F5F1E8]/60">
                    CLOSER.
                  </span>
                </h1>
              </div>
              ...
              <div className="md:col-span-7 md:justify-self-end">
                <p className="max-w-md text-[15px] leading-7 text-[#F5F1E8]/60 md:ml-auto md:text-right">
                  Pop-ups, private events, catering spreads,
                  and the food truck on the move — a look at
                  Karachi Flames wherever it shows up.
                </p>
              </div>
            </div>

            <div
              className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 md:auto-rows-[12rem] md:grid-cols-12 md:gap-5 xl:auto-rows-[15rem] xl:gap-6"
              data-gallery-reveal
            >
              {filteredItems.map((item, index) => (
                <article
                  key={item.id}
                  className={`min-w-0 ${galleryLayoutClasses[item.layout]}`}
                >
                  <button
                    type="button"
                    onClick={(event) =>
                    ...
                    className={`group relative isolate flex h-full min-h-[22rem] w-full overflow-hidden border border-white/10 bg-[#111] text-left shadow-[0_16px_45px_rgba(0,0,0,0.18)] transition duration-500 hover:border-[#D66A2B]/50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#D66A2B] ${item.radius}`}
          ...
          >
            <div className="relative z-10 flex items-center justify-between gap-4 border-b border-white/10 px-4 py-3 sm:px-6 sm:py-4">
              <p className="truncate text-[9px] font-bold uppercase tracking-[0.18em] text-[#F5F1E8]/55">
                ...
                of{" "}
                {String(filteredItems.length).padStart(
                  2,
                  "0"
                )}
              </p>
            ...
            >
              <div
                key={activeImage.id}
                className="gallery-lightbox-media relative h-full min-h-[18rem] w-full"
              >
                <GalleryMedia
                  item={activeImage}
                  sizes="100vw"
            ...
            <div className="flex flex-col gap-2 border-t border-white/10 px-4 py-4 sm:flex-row sm:items-end sm:justify-between sm:px-6 sm:py-5">
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-[#D66A2B]">
                  {activeImage.eyebrow}
                </p>

                <h2
                  id="image-lightbox-title"
                  className="mt-1 font-serif text-2xl tracking-[-0.045em] text-[#F5F1E8] sm:text-3xl"
                >
                  {activeImage.title}
                </h2>
              </div>
